...
\todo{Add information about the complexity of the problem when the structure is provided as a presentation (that is, a set of generators and a set of relations) instead of the full Cayley table.}
% task (focus on author) why me? - what was undertaken to address the need
We reduce the best upper bound on the complexity of the rank problem for semigroups, quasigroups, and groups and provide a theoretically efficient algorithm for the latter two.
% object (focus on document) why this document - what the document covers
This paper proves that with short certificates of correctness, the rank problem for quasigroups and groups can be verified by highly restricted models of computation.
...
\begin{lemma}\label{lem:ctt}
  ...
  If $L_1 \leq^{\NAC_0}_{ctt} L_2$ and
  \begin{enumerate}
  \item $L_2$ is in $\NP$, then $L_1$ is in $\NP$,
  \item $L_2$ is in $\NL$, then $L_1$ is in $\NL$.
  \end{enumerate}
  If $L_1 \leq^{\bAC_0}_{ctt} L_2$ and
  \begin{enumerate}[resume]
  \item $L_2$ is in $\FOLL$, then $L_1$ is in $\bFOLL$.
  \item $L_2$ is in $\L$, then $L_1$ is in $\bL$.
  \end{enumerate}
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
  This is essentially the same as item 2 of the previous lemma, except instead of looping over each of the $y_i$s, the machine $M_1$ can simply nondeterministically choose one $y_i$ and simulate $M_2$ on input $y_i$.
...
\begin{lemma}\label{lem:submagmamem}
...
%% The general strategy is to reduce the problem of computing the rank of a magma to the problem of determining whether an element of the magma can be generated from a given set of elements.

%% \begin{lemma}
%%   For any class of magmas $\mathcal{G}$, there is a $\beta_{\kappa(n) \log n} \AC_0$ conjunctive truth-table reduction from \textsc{Rank($\mathcal{G}$)} to \textsc{Membership($\mathcal{G}$)}, where $\kappa(n)$ is an upper bound on the size of a generating set for magmas in $\mathcal{G}$.
%% \end{lemma}
%% \begin{proof}
%%   Suppose the input to the reduction is $(G, k)$, where $G$ is a magma on $n$ elements in class $\mathcal{G}$ given as its Cayley table and $k$ is a unary integer.
%%   Let $G = \{g_1, \dotsc, g_n\}$.
%%   The reduction proceeds as follows.
%%   Nondeterministically choose a sequence $S$ of $\kappa(n)$ magma elements and a parenthesization $P$ of $S$.
%%   Output $((G, g_1, S, P), \dotsc, (G, g_n, S, P))$.

%%   How many nondeterministic bits are required by this reduction?
%%   Each magma element can be represented with $O(\log n)$ bits, so a sequence of $\kappa(n)$ such elements can be represented with $O(\kappa(n) \log n)$.
%%   A parenthesization of $\kappa(n)$ elements can be represented as a binary string of length $\kappa(n)$, so the overall number of nondeterministic bits required remains $O(\kappa(n) \log n)$.
%%   By definition, a magma $G$ has rank $k$ if there is a set of elements $S$ such that for each element $g$ of $G$ there is a parenthesization $P$ such that $g = \gen{S; P_i}$.

%%   By definition of rank,
%%   \begin{equation*}
%%     \rank(G) \leq k \iff \bigwedge_{i = 1}^n g_i \in \gen{S; P_i},
%%   \end{equation*}
...
\begin{proof}
  ...
  \begin{enumerate}
  \item
    Follows from \autoref{lem:ctt}, \autoref{lem:submagmamem}, and \autoref{lem:ranktomem}.
  \item
    Follows from \autoref{lem:ctt}, \autoref{lem:subsemigroupmem}, and \autoref{lem:ranktomem}.
  \item
    Follows from \autoref{lem:ctt}, \autoref{lem:cubemem}, and \autoref{lem:ranktomem}.
  \item
    Follows from \autoref{lem:ctt}, \autoref{lem:subgroupmem}, and \autoref{lem:ranktomem}, and from the previous item, since a group is a quasigroup.
  \end{enumerate}
\end{proof}
