...
The inputs are a semigroup $G$ given as a multiplication table, a semigroup element $h$, and a finite set $S$ of semigroup elements.
The problem is to decide whether $h \in \gen{S}$.

\todo{rename multiplication table to Cayley table}
\begin{lemma}\label{lem:subsemigroupmem}
  \textsc{Subsemigroup Membership} is in $\NL$.
\end{lemma}
\begin{proof}
  \todo{define disjunctive truth-table reduction}
  ...
  The multiplication table induces an edge-labeled directed multigraph $\Gamma$ with self-loops: the vertices of the graph are the semigroup elements, and for each semigroup element $a$, $b$, and $c$, there is an edge labeled $b$ from $a$ to $c$ if $a \cdot b = c$.
  ...
  Computing the adjacency matrix of $\Gamma_S$ from the multiplication table of $G$ and the set $S$ can be done in logarithmic space by setting each entry $(x, y)$ in the multiplication table to be zero if $y$ is not in $S$.
...
\begin{proof}
  ...
  On input $(G, k)$, where $G$ is a semigroup of order $n$ given as its multiplication table and $k$ is a positive integer given in binary,
