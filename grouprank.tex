...
%% TODO Should this text go somewhere else, or in a different section entirely?
Is it interesting to ask about the ``rank'' of finite rings or finite fields, and what does rank mean for these algebraic structures?
A finite ring comprises an additive group and a multiplicative monoid (with additional restrictions like distributivity), so the problem of computing the rank of a ring should be no harder than the problem of computing the rank of the group and the rank of the monoid.
Since our purpose for asking questions about the rank of an algebraic structure is to determine the smallest number of elements required to generate all other elements, we define the \emph{rank of a ring} to be the smaller of the rank of its additive group and the rank of its multiplicative monoid.

Since computing the rank of a group is in $\bFOLL \cap \bL$ and computing the rank of a monoid is in $\NL$ (because a monoid is a semigroup; see \autoref{thm:rank}), computing the rank of a ring, commutative or non-commutative, given by a pair of Cayley tables, is in $(\bFOLL \cap \bL) \cup \NL$, which is just $\NL$.
In the special case that the ring is a finite domain (that is, it has no nontrivial zero divisors), then by Wedderburn's Theorem \autocite[Theorem~3~§~11.1]{nicholson12}, the ring is a finite field.
The multiplicative group of any finite field is cyclic \autocite[Theorem~7~§~6.4]{nicholson12}, so the rank of a finite field, and hence a finite domain, is one and the computational problem is uninteresting.
