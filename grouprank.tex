...
\begin{proof}
  ...
  We will show a $\bAC_0$ conjunctive normal form truth-table reduction from $\textsc{Quasigroup Isomorphism}$ to the join of $\textsc{Cube Membership}$ and $\textsc{Product Equality}$, both of which are in $\FOLL$.
...
\todo{Open questions.}
\begin{itemize}
\item Are group membership and ring membership incomparable?
\item
  Is there a ring with rank strictly less than the minimum of the rank of its underlying group and monoid?
  ...
  If not, then the ring rank problem immediately becomes easier.
\item How hard is the succinct version of these problems?
\item
