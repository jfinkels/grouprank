...
\begin{example}
  ...
  Thus $G \times H$ has a generating set of strictly smaller than $S \times T$, for sufficiently large $m$ and $n$.
...
\section{Computing rank of other algebraic structures}
% Foreword
%
% context (focus on anyone) why now? - current situation, and why the need is so important
...
A finite ring comprises an additive group and a multiplicative monoid with an additional distributivity property.
...
The rank of a ring is bounded above by the minimum of the rank of its additive group and the rank of its multiplicative monoid, but in general it could be smaller.

Not all finite rings have an interesting rank problem.
...
For arbitrary rings, however, the problem is interesting.
We can apply the same strategy used for \textsc{Magma Rank} to determine the complexity of \textsc{Ring Rank}: reduce the rank problem to the membership problem, then show an upper bound on the membership problem.
...
\begin{proof}
  ...
  To simplify the closed-form representation of $r$, let $b_0 = -1$ and $a_{k + 1} = 1$.
  By construction of the graph $G$, we know $r = (\dotsb ((1 a_1 - b_1) a_2 - b_2) \dotsb) a_k - b_k$, or more concisely,
  \begin{equation*}
    r = \sum_{i = 0}^k -b_i \prod_{j = i + 1}^{k + 1} a_i.
  \end{equation*}
...
\begin{proof}
  ...
  Using the fact that $\bAC_0$ is a subset of $\NAC_0$, \autoref{lem:ringranktomem} and \autoref{lem:ctt} imply that \textsc{Ring Rank} is in $\NL$.
\end{proof}
