...
\todo{Where should this text go}

A finite ring is a pair of groups with additional restrictions, so the problem of computing the ``rank'' of a ring should be no harder than the problem of computing the rank of a group.
What does rank mean for these algebraic structures?
Since our purpose for asking questions about the rank of a magma is to determine the smallest number of elements required to generate all other elements, we will define the \emph{rank of a ring} to be the smaller of the rank of its additive group and the rank of its multiplicative group.

Since a ring, commutative or non-commutative, comprises an abelian group under the additive ring operation, the rank problem for finite rings (given as a pair of Cayley tables) reduces to the problem of determining the rank of a finite abelian group, which is in $\bFOLL \cap \bL$, as stated above.
If the ring is a finite domain (that is, it has no nontrivial zero divisors), then by Wedderburn's Theorem \autocite[Theorem~3~§~11.1]{nicholson12}, the ring is a finite field.
