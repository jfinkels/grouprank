...
\begin{proof}
  ...
  If $n_i$ is defined as above, the set $\{g_1^{n_i}, g_2^{n_i}, \dotsc, g_k^{n_i} \}$ is a generating set for $S_{p_i}$.
...
Now that we know the complexity for the decision problem \textsc{Min Gen Size(Group)}, we wish to implement a function which computes the exact size of the minimum generating set of a group.
For decision problems in polynomial time, the well-known binary search on $k$ between 1 and $n$, the order of the group, places the search problem in polynomial time as well.
