...
TODO Discuss previous work here, including Barrington and McKenzie showing the group membership problem is in $\SL$, and the $\DSPACE(\log^2 n)$ algorithm, the work Graph Isomorphism is not AC0 reducible to Group Isomorphism, which shows that Quasigroup Isomorphism is also in $\GC(\log^2 n, \FOLL)$, \ldots.
