...
In the first phase, we use in parallel exactly $\log n$ instances of the original \FOLL{} circuit for deciding \textsc{Min Gen Size} with the value of $k$ hardcoded; one instance has $k = 1$, one has $k = 2$, etc.
...
The minimum of $\log n$ numbers, each of length at most $\log \log n$, can be computed by an unbounded fan-in circuit over $\{\AND, \OR, \NOT\}$ of size polynomial in $\log n$ and depth $O(\log \log n)$ and since the depth of the circuit in the first phase is $O(\log \log n)$, the overall depth remains $O(\log \log n)$.
