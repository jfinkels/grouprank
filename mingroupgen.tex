...
\end{todo}
%% This proof is from \url{http://math.stackexchange.com/a/226938/29369}.
\begin{proof}
...
The elementary abelian groups are isomorphic to $(\mathbb{Z}/p\mathbb{Z})^m$ for some prime $p$ and some non-negative integer $m$.
...
\end{lemma}
\begin{proof}
  The following algorithm computes $m$ from the (unary) input $1^n$ where $n = p^m$ for some $p$ and $m$.
  ...
  In the third step, deciding if a number is prime can be done in \NC{}, again since the input is given in unary, and using an efficient deterministic polynomial time algorithm like the AKS primality test \cite{aks04}.
...
Now that we know the complexity for the decision problem form of \textsc{Min Gen Size($\mathcal{G}$)}, we wish to implement a function which computes the exact size of the minimum generating set of a group.
As noted after \autoref{lem:log}, we need only search for a generating set of size at most $\log n$ for groups of order $n$.

For the subclasses of \textsc{Group} for which the decision problem is in polynomial time, the well-known binary search on $k$ between 1 and $\log n$, places the search problem in polynomial time as well.
...
Since the depth of the recursion tree is $\log \log n$, the search problem is computable in $\GC(\log^3 n, \L)$.

Now consider a decision problem in $\GC(\log^2 n, \FOLL)$.
In order to search for the exact solution, we construct a circuit that works in two phases.
In the first phase, we have exactly $\log n$ instances of the original \FOLL{} circuit for deciding \textsc{Min Gen Size} with the value of $k$ hardcoded; one instance has $k = 1$, one has $k = 2$, etc.
Each of these instances has concurrent read-only access to both the group $G$ and the $\log^2 n$ bits of nondeterminism.
Each instance will use only the first $k \log n$ nondeterministic bits, since it only needs to choose a set of size $k$ in which each element has size $\log n$.
Each instance outputs $k$ (in binary) if $G$ has a generating set of size $k$ and outputs $\log n$ (in binary) if not.
In the second phase we have a circuit computing the minimum of each of the $\log n$ values output from the previous phase.

The minimum of $\log n$ numbers, each of length at most $\log \log n$, can be computed by an unbounded fan-in circuit over $\{\AND, \OR, \NOT\}$ of size polynomial in $\log n$ and depth $O(\log \log n)$ \cite{citationneeded} and since the depth of the circuit in the first phase is $O(\log \log n)$, the overall depth remains $O(\log \log n)$.
The first phase contributes a $O(\log n)$ multiplicative factor to the size of the circuit, so the circuit remains polynomial in $n$.
We conclude that if the decision form of \textsc{Min Gen Size} is in $\GC(\log^2 n, \FOLL)$, so is the search form.

The same argument applies to decision problems in $\GC(\log^2 n, \FOLL^2)$, and essentially the same argument (with the exception of the nondeterminism) applies to those in \NC.
...
\caption{A summary of the upper bounds due to this work for the problem of computing the size of the minimum generating set of a class of groups.\label{fig:results}}
  \begin{center}
    \begin{tabular}{l | l}
      ...
      \textsc{Nilpotent} & $\GC(\log^3 n, \L) \cap \GC(\log^2 n, \FOLL^2) \cap \P$ \\
      \textsc{p-Group} & $\GC(\log^3 n, \L) \cap \GC(\log^2 n, \FOLL^2) \cap \P$ \\
      \textsc{Constant Solvability Class} & $\GC(\log^3 n, \L) \cap \GC(\log^2 n, \FOLL)$ \\
      \textsc{Abelian} & $\GC(\log^3 n, \L) \cap \GC(\log^2 n, \FOLL) \cap \P$ \\
      \textsc{Elementary Abelian} & $\GC(\log^3 n, \L) \cap \GC(\log^2 n, \FOLL) \cap \NC$ \\
      \textsc{Cyclic} & $\NC^0$
    \end{tabular}
  \end{center}
\end{figure}
