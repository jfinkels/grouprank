...
The rank of a ring is bounded above by the minimum of the rank of its additive group and the rank of its multiplicative monoid.  % We are not sure if there is an example of a finite ring that has strictly smaller ring rank than group or monoid rank.
%% *This is not necessarily true*: Indeed, the \textsc{Ring Rank} problem is in $\P$, since a ring is an abelian group under addition, and for abelian groups (even for nilpotent groups) a minimum generating set can be computed in polynomial time \autocite[Theorem~7]{at06}.
