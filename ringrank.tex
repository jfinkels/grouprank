...
We represent it as a pair of Cayley tables, one for the group and one for the monoid.\footnote{
  ...
  This is the representation chosen in, for example, \autocite{ks06} and \autocite{av05} when designing algorithms for finite rings.
}
...
The hardness of this problem lies in computing the rank of the underlying monoid (which is just a semigroup with an identity element, so the worst-case complexity of the rank problem for monoids and for semigroups is the same).
...
\begin{theorem}\label{thm:ringrank}
  \textsc{Ring Rank} is in $\bAC^1 \cap \P$.
\end{theorem}
\begin{proof}
