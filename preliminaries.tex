...
We assume familiarity with the basics of computational complexity theory, as can be found in \autocite{ab09}, for example.
We use the standard definitions of the complexity classes $\P$, $\L$, $\NL$, and $\AC$, as well as the standard definition of many-one reductions.
We denote by $\L^2$ the class of languages decidable in $O(\log^2 n)$ space by $\L^2$.

We denote by $\NL[\log^2 n]$ the subclass of $\NL$ in which the number of nondeterministic choices is bounded by $O(\log^2 n)$.
The class $\bL$ is the superclass of $\NL[\log^2 n]$ in which the Turing machine has two-way read-only access to the input string, \emph{two-way} read-only access to the tape containing the witness string, and two-way read-write access to the work tape.
Equivalently, $\bL$ is the class of languages $A$ such that there is a language $A' \in \L$ such that $x \in A$ if and only if there is a binary string $w$ of length $O(\log^2 n)$ such that $(x, w) \in A'$.
...
$\bAC^0$ is the restriction of $\bFOLL$ to depth $O(1)$.
The class $\NAC^0$ is $\AC^0$ augmented with a polynomial number of nondeterministic bits.
...
The proof is a straightforward composition of the function computing the reduction with the machine that decides the language $L_2$.
...
\begin{proof}
  ...
  One way for a circuit to compute the product of two quasigroup elements using the Cayley table is via a \emph{multiplexer}.
